...
<template>
    <Popover v-slot="{ open }" class="relative">
        <PopoverButton
            :class="open ? '' : ''"
            class="group inline-flex items-center py-2"
        >
                ...
                >
                    <div class="relative grid gap-8 bg-white bg-opacity-50 backdrop-blur p-7 lg:grid-cols-2">
                        <Link
                            ...
                            :key="item.name"
                            :href="item.href"
                            class="-m-3 flex items-center rounded-2xl p-2 transition duration-150 ease-in-out hover:bg-gray-300 hover:bg-opacity-50"
                        >
                            <i class="fa-regular text-2xl ml-2" :class="item.icon" />
                            <div class="ml-4">
